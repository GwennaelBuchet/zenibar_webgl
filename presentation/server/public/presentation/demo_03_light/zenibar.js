...
let useDiffuseLight = false;
let useSpecularLight = false;
let gl;
...
function main() {
	...
	const shaderProgramParams = {
		...
		useAmbientLight: gl.getUniformLocation(shaderProgram, 'uUseAmbientLight'),
		useDiffuseLight: gl.getUniformLocation(shaderProgram, 'uUseDiffuseLight'),
		useSpecularLight: gl.getUniformLocation(shaderProgram, 'uUseSpecularLight')
	};
...
function initCubeBuffers(gl) {
	...
	// Buffer for normals of each vertex of each face
 ...
 * @param buffers {{position: WebGLBuffer, color: WebGLBuffer, indices: WebGLBuffer, normals: WebGLBuffer}}
 */
...
// let's move the scene to -10 along Z axis (as if we moved the camera to +10 on Z)
	...
	// Set the vertexNormal attribute of the shader
	...
	gl.uniform1i(
		shaderProgramParams.useAmbientLight,
		useAmbientLight);
	gl.uniform1i(
		shaderProgramParams.useDiffuseLight,
		useDiffuseLight);
	gl.uniform1i(
		shaderProgramParams.useSpecularLight,
		useSpecularLight);
	gl.uniformMatrix4fv(
